...
def main():
	...
	create_plots_for_service_multicharts(service_usage_data, max_monthly_cost, "monthly", MONTHLY_START_DATE, MONTHLY_END_DATE)
	create_plots_for_service_usage_multicharts(service_usage_data, max_monthly_cost, "monthly", MONTHLY_START_DATE, MONTHLY_END_DATE)
	...
	create_plots_for_service_multicharts(service_usage_data, max_cost, "daily", DAILY_START_DATE, DAILY_END_DATE)
	create_plots_for_service_usage_multicharts(service_usage_data, max_cost, "daily", DAILY_START_DATE, DAILY_END_DATE)
...
def get_and_write_costs_to_files():
	...
	costs = get_costs_for_group(DAILY_START_DATE, DAILY_END_DATE, "DAILY", ["SERVICE", "USAGE_TYPE"] )
	formatted_json = json.dumps(costs, indent=3, default=str)
	with open("results_sample_cost.json", "w") as file:
		file.write(formatted_json)
	formatted = format_costs(costs)
...
def create_plots_for_service_multicharts(service_usage_data, max_monthly_cost, filename_qualifier, start, end):
